...
class Account(models.Model):
    ...
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name="account")
    acc_num = models.CharField(max_length=30, primary_key=True)  # 계좌번호 PK
    acc_pwd = models.PositiveIntegerField()           # 계좌 비밀번호 (6자리)
    acc_type = models.PositiveIntegerField(choices=acc_choices)     # 계좌 타입 (은행, 증권, 금)
    acc_name = models.CharField(max_length=20)        # 계좌명
    total = models.DecimalField(null=True, blank=True, max_digits=11, decimal_places=4)       # 총 잔액
    created_at = models.DateField()  # 계좌 개설일자
    updated_at = models.DateField(auto_now=True)      # 최근 거래일자
    acc_detail = models.PositiveIntegerField(choices=acc_detail)   # 계좌 디테일
...
class Trans(models.Model):
    ...
    trans_price = models.DecimalField(max_digits=11, decimal_places=4)          # 거래 금액
    trans_content = models.CharField(max_length=30)                             # 거래 내용 
    trans_type = models.PositiveIntegerField(choices=trans_choices)   
